...
public class BallScript : MonoBehaviour
{
    ...
    {
        if (gameData.sound)
            audioSrc.PlayOneShot(loseSound, 5 * PlayerPrefs.GetFloat("SoundEffectsVolumeCoef"));
        Destroy(gameObject);
    ...
    {
        if (gameData.sound)
            audioSrc.PlayOneShot(hitSound, 5 * PlayerPrefs.GetFloat("SoundEffectsVolumeCoef"));
    }
}
